...
from ocr.nets.crnn import CRNN
from ocr.config import common_config
from ocr.ctc_decoder import ctc_decode
from ocr.dataset import LicensePlate

'''
训练自己的数据集必看注释！
'''
pretrained_path = './ocr/checkpoint/crnn_25500_loss0.050892.pth'
...
class YOLO(object):
    ...
    def generate(self, onnx=False):
        ...
        '''
            建立CRNN模型
        '''
        self.crnn = CRNN(input_c=common_config['img_channel'],
                         input_h=common_config['img_height'], num_classes=common_config['num_classes'] + 1)
        self.crnn.load_state_dict(torch.load(
            pretrained_path, map_location=device))
        self.crnn.to(device).eval()
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            '''
                识别车牌
            '''
            crop_image = image.crop([left, top, right, bottom]).convert('L')
            crop_image = transform(crop_image).unsqueeze(0)  # 增加batch维度
            with torch.no_grad():
                crop_image = crop_image.to(device)
                logits = self.crnn(crop_image)
    ...
    def detect_heatmap(self, image, heatmap_save_path):
        ...
        for sub_output in outputs:
            ...
            sub_output = np.transpose(np.reshape(
                sub_output, [b, 3, -1, h, w]), [0, 3, 4, 1, 2])[0]
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            f.write("%s %s %s %s %s %s\n" % (predicted_class, score[:6], str(
                int(left)), str(int(top)), str(int(right)), str(int(bottom))))
